...
public class ClassUtil {
	 ...
	 * 
	 * @param obj
	 *            the object the specified method is invoked from (use a Class
	 *            object if method is static)
	 * @param name
	 *            method name
	 * 
	 ...
	 *         are logged)
	 */
	public static Object invokeMethod(Object obj, String name, Class param[],
			Object args[]) {
		...
		try {
			...
			// log.warn("No method "+(obj instanceof Class ? (Class)obj :
			// obj.getClass()).toString()+"."+name+"("+Arrays.asList(param)+") found - "+TnesException.firstStackTrace(x));
			return null;
		} catch (Exception x) {
			Logger log = Logger.getLogger(ClassUtil.class);
			log.error("", x);
