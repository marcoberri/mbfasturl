...
public class IOUtil {
	...
	/**
	 * Returns the absolute path of a directory checking it against
	 * MicroKernel.getEnv("root"). Like absoluteDirectory(path,null)
	 * 
	 * @param path
	 *            file to check
	...
	/**
	 * Returns the absolute path of a directory checking it against 'root' Calls
	 * {@link absolutePath(String,String)} and than appends a trailing "/"
	 * 
	 * @param path
	 *            file to check
	 * @param root
	 *            the root: must be absolute
	 * @return the absolute path or null if root is not absolute or path is
	 *         absolute and doesn't start with root
	 */
	...
	/**
	 * Returns the absolute path of a file checking it against
	 * MicroKernel.getEnv("root"). Like absolutePath(path,null)
	 * 
	 * @param path
	 *            file to check
	 * @return the absolute path
	 ...
	 *            file to check
	 * @param root
	 *            the root: must be absolute (if null then
	 *            MicroKernel.getEnv("root") will be used)
	 * @return the absolute path or null if root is not absolute or path is
	 *         absolute and doesn't start with root
	 */
	...
	/**
	 * Returns the canonical path of a file or null if an exception occours or
	 * the file is out of root
	 * 
	 * @param path
	 *            file to check
	 * @param root
	 ...
	 * 
	 * @param regex
	 *            String a con espressione regolare
	 * @param dir
	 *            la cartella su cui eseguire la dir
	...
	/**
	 * Finds a file starting from a relative path (parent), a resource path
	 * (file) without exiting from Tnes root.
	 * 
	 * @param parent
	 * @param file
	 * @return
	...
	/**
	 * Finds a file starting from a relative path (parent), a file path array
	 * (files) without exiting from a root.
	 * 
	 * @param parent
	 * @param files
	 * @param root
	...
	/**
	 * Finds a file starting from a relative path (parent), a file path (file)
	...
	/**
	 * Finds a file starting from a relative path (parent), a file path array
	 * (files) without exiting from Tnes root.
	 * 
	 * @param parent
	 * @param files
	 * @return
	...
	/**
	 * Returns the normalized path of a directory. Calls {@link
	 * normalizedPath(String)} and than appends a trailing "/"
	 * 
	 * @param path
	 *            dir to normalize
	...
	/**
	 * Returns the normalized path of a file. A normalized path is a path where
	 * path separator is "/" and no redundant elements (like "." and "..") are
	 * present.<br/>
	...
	public static final String normalizedPath(String path) {
		...
		for (File file = new File(path.trim().replace('\\', '/')); file != null; file = file
				.getParentFile()) {
			stack.push(file);
		}
	...
	public static final String parentPath(String path) {
		...
		return normalizedDirectory(Default.notEmpty(
	 ...
	 *            Reader to read
	 * @param nLines
	 *            number of lines to read, if nLines <= 0 then read until end of
	 *            reader
	 * @return an array of String containing reader's lines
	 */
	...
	/**
	 * Reads a reader starting from startLine and putting nLines lines in an
	 * array.
	 * 
	 * @param reader
	 *            Reader to read
	 * @param startLine
	 *            line to start, if startLine < 0 then startLine = 0
	 * @param nLines
	 *            number of lines to read, if nLines <= 0 then read until end of
	 *            reader
	 * @return an array of String containing reader's lines
	 */
	public static final String[] readLines(Reader reader, int startLine,
			int nLines) {
		if (reader == null) {
			return null;
		}
		...
		try {
			...
			int i = 0;
			for (String s = null; (nLines <= 0 || i < nLines)
					&& (s = buf.readLine()) != null; i++) {
				if (i < startLine) {
					continue;
				}
				lines.add(s);
