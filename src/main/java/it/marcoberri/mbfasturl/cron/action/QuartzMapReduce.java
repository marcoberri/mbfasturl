...
public class QuartzMapReduce implements Job {
     ...
     *
     */
	protected static final org.apache.log4j.Logger log = Log4j.getLogger(
			QuartzMapReduce.class.getSimpleName(), ConfigurationHelper
					.getProp().getProperty("log.path"), Log4j.ROTATE_DAILY);
	...
	private String getJsFromFile(String fileName, Logger log) {
		...
		try {
			final URL main = log.getClass().getProtectionDomain()
					.getCodeSource().getLocation();
			final String path = URLDecoder.decode(main.getPath(), "utf-8");
			final String webInfFolderPosition = new File(path).getPath();
			final String webInfFolder = webInfFolderPosition.substring(0,
					webInfFolderPosition.indexOf("classes"));
			return FileUtils.readFileToString(new File(webInfFolder
					+ File.separator + fileName));
		} catch (final IOException ex) {
			Commons.writeEventLog("getJsFromFile", false, ex.getMessage(),
					"MapReduce", (System.currentTimeMillis() - ts));
			log.error(ex);
		}
		return null;
	}
	...
	private void mapReduceCountAllDayClick(Logger log) {
		...
		final String map = getJsFromFile("/mapreduce/stats/" + l + "/map.txt",
				log);
		final String reduce = getJsFromFile("/mapreduce/stats/" + l
				+ "/reduce.txt", log);
		final MapreduceResults<StatsCountTimeDayClick> mrRes = ds.mapReduce(
				MapreduceType.MERGE, ds.createQuery(Log.class), map, reduce,
				null, null, StatsCountTimeDayClick.class);
	...
	private void mapReduceCountSingleBrowser(Logger log) {
		...
		final String reduce = getJsFromFile("/mapreduce/stats/" + l
				+ "/reduce.txt", log);
		final MapreduceResults<StatsCountSingleBrowser> mrRes = ds.mapReduce(
				MapreduceType.MERGE, ds.createQuery(Log.class), map, reduce,
				null, null, StatsCountSingleBrowser.class);
		writeLogMR(l, mrRes, (System.currentTimeMillis() - ts));
	}
	...
	private void mapReduceCountSingleBrowserUrl(Logger log) {
		...
		final String reduce = getJsFromFile("/mapreduce/stats/" + l
				+ "/reduce.txt", log);
		final MapreduceResults<StatsCountSingleBrowserUrl> mrRes = ds
				.mapReduce(MapreduceType.MERGE, ds.createQuery(Log.class), map,
						reduce, null, null, StatsCountSingleBrowserUrl.class);
		writeLogMR(l, mrRes, (System.currentTimeMillis() - ts));
	}
	...
	private void mapReduceCountSingleCountryUrl(Logger log) {
		...
		final MapreduceResults<StatsCountSingleCountryUrl> mrRes = ds
				.mapReduce(MapreduceType.MERGE, ds.createQuery(Log.class), map,
						reduce, null, null, StatsCountSingleCountryUrl.class);
	...
	private void mapReduceCountSingleIp(Logger log) {
		...
		final String reduce = getJsFromFile("/mapreduce/stats/" + l
				+ "/reduce.txt", log);
		log.debug("reduce:" + reduce);
		final MapreduceResults<StatsCountSingleIp> mrRes = ds.mapReduce(
				MapreduceType.MERGE, ds.createQuery(Log.class), map, reduce,
				null, null, StatsCountSingleIp.class);
		log.debug("count:" + mrRes.getClass());
		writeLogMR(l, mrRes, (System.currentTimeMillis() - ts));
	}
	...
	private void mapReduceCountSingleSite(Logger log) {
		...
		final String reduce = getJsFromFile("/mapreduce/stats/" + l
				+ "/reduce.txt", log);
		log.debug("reduce:" + reduce);
		final MapreduceResults<StatsCountSingleSite> mrRes = ds.mapReduce(
				MapreduceType.MERGE, ds.createQuery(Log.class), map, reduce,
				null, null, StatsCountSingleSite.class);
		log.debug("count:" + mrRes.getClass());
		writeLogMR(l, mrRes, (System.currentTimeMillis() - ts));
	}
	...
	private void mapReduceCountSingleUrl(Logger log) {
		...
		final String reduce = getJsFromFile("/mapreduce/stats/" + l
				+ "/reduce.txt", log);
		log.debug("reduce:" + reduce);
		final MapreduceResults<StatsCountSingleUrl> mrRes = ds.mapReduce(
				MapreduceType.MERGE, ds.createQuery(Log.class), map, reduce,
				null, null, StatsCountSingleUrl.class);
		log.debug("count:" + mrRes.getClass());
		writeLogMR(l, mrRes, (System.currentTimeMillis() - ts));
	}
	...
	private void mapReduceCountTimeDayUrl(Logger log) {
		...
		final String reduce = getJsFromFile("/mapreduce/stats/" + l
				+ "/reduce.txt", log);
		final MapreduceResults<StatsCountTimeDayUrl> mrRes = ds.mapReduce(
				MapreduceType.MERGE, ds.createQuery(Log.class), map, reduce,
				null, null, StatsCountTimeDayUrl.class);
		writeLogMR(l, mrRes, (System.currentTimeMillis() - ts));
	}
	...
	private void mapReduceCountTimeMonthCountryUrl(Logger log) {
		...
		final String reduce = getJsFromFile("/mapreduce/stats/" + l
				+ "/reduce.txt", log);
		final MapreduceResults<StatsCountTimeMonthCountryUrl> mrRes = ds
				.mapReduce(MapreduceType.MERGE, ds.createQuery(Log.class), map,
						reduce, null, null, StatsCountTimeMonthCountryUrl.class);
		writeLogMR(l, mrRes, (System.currentTimeMillis() - ts));
	}
	...
	private void mapReduceCountTimeMonthUrl(Logger log) {
		...
		final String reduce = getJsFromFile("/mapreduce/stats/" + l
				+ "/reduce.txt", log);
		final MapreduceResults<StatsCountTimeMonthUrl> mrRes = ds.mapReduce(
				MapreduceType.MERGE, ds.createQuery(Log.class), map, reduce,
				null, null, StatsCountTimeMonthUrl.class);
		writeLogMR(l, mrRes, (System.currentTimeMillis() - ts));
	}
	...
	private void mapReduceCountTimeYearUrl(Logger log) {
		...
		final String reduce = getJsFromFile("/mapreduce/stats/" + l
				+ "/reduce.txt", log);
		final MapreduceResults<StatsCountTimeYearUrl> mrRes = ds.mapReduce(
				MapreduceType.MERGE, ds.createQuery(Log.class), map, reduce,
				null, null, StatsCountTimeYearUrl.class);
		writeLogMR(l, mrRes, (System.currentTimeMillis() - ts));
	}

	private void writeLogMR(String action, MapreduceResults mr, long ts) {
		Commons.writeEventLog(action, true, "tot: "
				+ mr.createQuery().countAll(), "MapReduce", ts);
	}
}
